...
1.  **Server-Side**: The backend (e.g., Express) uses `@miolab/openknowledge-core` to load configs and talk to LLMs securely.
2.  **Client-Side**: The frontend uses `@miolab/openknowledge-react` to display the widget, but does NOT contain API keys or sensitive logic.
